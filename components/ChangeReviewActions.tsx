...
import { approveChangeByToken, rejectChangeByToken } from "@/app/actions/pendingChanges";

type DoneState = "approved" | "rejected" | null;

export function ChangeReviewActions({ token }: { token: string }) {
  ...
  async function run(action: "approve" | "reject") {
    setPending(action);
    setError(null);
    const res = action === "approve" ? await approveChangeByToken(token) : await rejectChangeByToken(token);
    setPending(null);
    if (!res.ok) {
  ...
  if (done) {
    return (
      <div className="rounded-md bg-green-500/10 px-4 py-3 text-sm font-medium text-green-300">
        {done === "approved"
          ? "Change approved — it's now live on the site."
          : "Change rejected — the site stays as it was."}
      </div>
    );
  }
